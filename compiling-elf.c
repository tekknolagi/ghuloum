...
uword Buffer_entrypoint(Buffer *buf) { return buf->entrypoint; }
void Buffer_set_entrypoint(Buffer *buf, uword entrypoint) {
...
void Buffer_write16(Buffer *buf, uint16_t value) {
  for (uword i = 0; i < sizeof(value); i++) {
    Buffer_write8(buf, (value >> (i * kBitsPerByte)) & 0xff);
  }
}

void Buffer_write32(Buffer *buf, uint32_t value) {
  for (uword i = 0; i < sizeof(value); i++) {
    Buffer_write8(buf, (value >> (i * kBitsPerByte)) & 0xff);
  }
}
...
void Buffer_at_put16(Buffer *buf, word offset, uint16_t value) {
  for (uword i = 0; i < sizeof(value); i++) {
    Buffer_at_put8(buf, offset + i, (value >> (i * kBitsPerByte)) & 0xff);
  }
}

void Buffer_at_put32(Buffer *buf, word offset, uint32_t value) {
  for (uword i = 0; i < sizeof(value); i++) {
    Buffer_at_put8(buf, offset + i, (value >> (i * kBitsPerByte)) & 0xff);
  }
}
...
WARN_UNUSED int Compile_labels(Buffer *buf, ASTNode *bindings, ASTNode *body,
  ...
  if (AST_is_nil(bindings)) {
    Buffer_set_entrypoint(buf, Buffer_len(buf));
...
WARN_UNUSED int Compile_entry(Buffer *buf, ASTNode *node) {
  ...
  Emit_shr_reg_imm8(buf, kRax, kIntegerShift);
  Emit_mov_reg_reg(buf, kRdi, kRax); // return code from result of body
  Emit_mov_reg_imm32(buf, kRax, 60); // exit
  Emit_syscall(buf);
  // Buffer_write_arr(buf, kFunctionEpilogue, sizeof kFunctionEpilogue);
  return 0;
}

WARN_UNUSED int Compile_to_file(ASTNode *node, const char *filename) {
  (void)node;
  Buffer buf;
  Buffer_init(&buf, 1);
  ...
  // BEGIN e_ident
  Buffer_write8(&buf, 0x7F);
  Buffer_write8(&buf, 'E');
  Buffer_write8(&buf, 'L');
  Buffer_write8(&buf, 'F');
  Buffer_write8(&buf, 2); // 64-bit
  Buffer_write8(&buf, 1); // little-endian
  Buffer_write8(&buf, 1); // ???
  Buffer_write8(&buf, 0); // ???
  for (word i = 0; i < 8; i++) {
    Buffer_write8(&buf, 0); // unused e_ident
  }
  // END e_ident
  Buffer_write32(&buf, 0x003e0002); // e_machine_type
  Buffer_write32(&buf, 1);          // e_version
  word e_entry_pos = Buffer_len(&buf);
  Buffer_write64(&buf, 0); // e_entry
  word e_phoff_pos = Buffer_len(&buf);
  Buffer_write64(&buf, 0); // e_phoff
  Buffer_write64(&buf, 0); // e_shoff
  Buffer_write32(&buf, 0); // e_flags
  word e_ehsize_pos = Buffer_len(&buf);
  Buffer_write16(&buf, 0); // e_ehsize
  word e_phentsize_pos = Buffer_len(&buf);
  Buffer_write16(&buf, 0); // e_phentsize
  Buffer_write16(&buf, 1); // e_phnum
  Buffer_write16(&buf, 0); // e_shentsize
  Buffer_write16(&buf, 0); // e_shnum
  Buffer_write16(&buf, 0); // e_shstrndx

  // Backpatch e_ehsize
  Buffer_at_put32(&buf, e_ehsize_pos, Buffer_len(&buf));

  word phdr_start_pos = Buffer_len(&buf);
  // Backpatch e_phoff
  Buffer_at_put64(&buf, e_phoff_pos, phdr_start_pos);

  Buffer_write32(&buf, 1);   // p_type
  Buffer_write32(&buf, 5);   // p_flags
  Buffer_write64(&buf, 0);   // p_offset
  Buffer_write64(&buf, org); // p_vaddr
  Buffer_write64(&buf, org); // p_paddr
  word filesz_pos = Buffer_len(&buf);
  Buffer_write64(&buf, 0); // p_filesz
  word memsz_pos = Buffer_len(&buf);
  Buffer_write64(&buf, 0);      // p_memsz
  Buffer_write64(&buf, 0x1000); // p_align

  word phdrsize = Buffer_len(&buf) - phdr_start_pos;
  // Backpatch e_phentsize
  Buffer_at_put16(&buf, e_phentsize_pos, phdrsize);

  _(Compile_entry(&buf, node));

  // Backpatch e_entry
  Buffer_at_put64(&buf, e_entry_pos, org + buf.entrypoint);

  word filesize = Buffer_len(&buf);
  // Backpatch p_filesz
  Buffer_at_put64(&buf, filesz_pos, filesize);
  // Backpatch p_memsz
  Buffer_at_put64(&buf, memsz_pos, filesize);

  FILE *fp = fopen(filename, "w+b");
  assert(fp != NULL);
  size_t written =
      fwrite(buf.address, /*size=*/1, /*nmemb=*/Buffer_len(&buf), fp);
  assert(written == (size_t)Buffer_len(&buf));
  fclose(fp);
  return 0;
}
...
SUITE(compiler_tests) {
  ...
  RUN_BUFFER_TEST(compile_multilevel_labelcall);
...
int main(int argc, char **argv) {
  assert(argc == 2 && "need filename and only filename");
  const char *filename = argv[1];
  ASTNode *program = Reader_read(
      "(labels ((factorial (code (x) "
      "            (if (< x 2) 1 (* x (labelcall factorial (- x 1)))))))"
      "    (labelcall factorial 5))");
  int code = Compile_to_file(program, filename);
  AST_heap_free(program);
  return code;
}
